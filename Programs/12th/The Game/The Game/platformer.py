...
class Game:
    def __init__(self):
        pyxel.init(WINDOW_SIZE, WINDOW_SIZE, fps=60)
    ...
    def update(self):
        ...
        with open("Values.csv","w") as csvfile:
            ...
            for i in range(8):
                writer.writerow([i+1,self.all_coins[i],self.max_coins[i],self.all_times[i],self.all_retries[i]])
    ...
    def draw(self):
        ...
        if self.screen=="finish_screen":
            ...
            with open('DeNiro.csv','r') as file:
                ...
                for row in reader:
                    pyxel.text(40, 100 + 10*i, row[0] + "        |   " + str(round(row[3],3)) + "  |      " + str(row[1]) + "/" + str(row[2]) + "    |      " + str(row[4]) , 8)
                score = 0
                for i in range(8):
